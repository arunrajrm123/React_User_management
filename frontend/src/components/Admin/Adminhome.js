...
const Adminhome = () => {
  ...
  const editUserHandler = () => {
    ...
    console.log("dey edit cheythu",admin.value.editactive) 
    ...
    <div>
      ...
      {searchState && (
        <>
          ...
          />
          <button class="btn-btn" onClick={handleSearch}> Search</button>
        </>
      )}

      <button
        className="btn-btn" style={{color:"red"}}
        onClick={() => {
      ...
      {deleteState ? (
        <div className="addusercard">
          ...
          <button onClick={deleteHandlerfunc} className="btn-btn">Confirm</button>
        </div>
      ) : (
        ""
      )}
      ...
      {addUserState ? (
        <div className="container mt-5" style={{ border: "2px solid black",width:400 }}>
          ...
          <button onClick={addUserHandler} className="btn-btn">
            Add User
          </button>
        </div>
      ) : (
          ...
          <tbody>
            {userDatas.map((item) => {
              return (
                <tr>
                  <td>{item.name}</td>
                  <img src={item.image} alt={`Image ${item.image}`} width="100" height="100" />
                  <td>{item.email}</td>
